...
**Важное про тип возврата.** Документация axum: у `Serve` (без graceful shutdown) футура резолвится в `io::Result<()>`, но **никогда не завершается и никогда не возвращает ошибку** — ошибки сокета обрабатываются сном и повтором. У `WithGracefulShutdown` `Output = ()`. Поэтому `wakode_api::serve` меняет тип возврата с `std::io::Result<()>` на `()`: `Result`, который не может быть ошибкой, — ложное обещание вызывающему.
...
///
/// Тип возврата — `()`, а не `io::Result<()>`. У axum без graceful
/// shutdown футура объявлена как `io::Result<()>`, но её документация
/// говорит прямо: она никогда не завершается и никогда не возвращает
/// ошибку — ошибки сокета обрабатываются сном и повтором приёма. `Result`,
/// который не может быть ошибкой, обязывает вызывающего писать `?` там,
/// где ветки отказа не существует.
///
...
Run: `cargo test -p wakode --lib signal`
