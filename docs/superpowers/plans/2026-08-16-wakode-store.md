...
**Architecture:** Синхронный `rusqlite` внутри, асинхронный трейт снаружи. Поток отметок идёт через единственную пишущую задачу с каналом и подтверждением по `oneshot` — это снимает борьбу за блокировку там, где она была бы массовой, и открывает дорогу групповой фиксации, когда она понадобится. Чтения, а также редкие одиночные записи (пользователи, ключи, сессии) идут мимо неё, по отдельным соединениям в WAL-режиме, через `spawn_blocking`: логин не должен ждать за чужим батчем отметок, а разводит такие соединения сам SQLite по `busy_timeout`. Строковые атрибуты хранятся номерами, словарь целиком живёт в памяти.
...
- Тесты `store` — интеграционные и **не утверждают ни текста SQL, ни формы таблиц** (спека §9). Сквозной набор через репозиторный трейт появляется в задаче 12; тесты задач 6–11 зовут функции напрямую, потому что трейта на тот момент ещё нет. Оговорка по итогу: через трейт в волне 0 проходят только `HeartbeatRepo` и `UserRepo` — `KeyRepo`, `SessionRepo`, `HeartbeatRepo::resolve` и `UserRepo::user_by_id` не вызывает ни один тест. Тела этих методов — одна строка одного и того же `on_own_connection`, так что переезд на Postgres они не удержат сами по себе; называть набор гарантией переезда преждевременно, пока это так. SQL разрешён ровно в трёх местах, и все три проверяют саму схему, а не поведение поверх неё: состав таблиц, уникальность индекса `hb_dedup` и срабатывание `ON DELETE CASCADE`.
...
/// Знать про SQL в этом файле позволено только проверкам самой схемы —
/// они проверяют её, а не поведение поверх неё. Этот тест — одна из них.
#[test]
...
Плюс два теста, которых нет в блоке кода выше, но которые должны появиться: `two_heartbeats_with_the_same_entity_but_different_projects_are_both_inserted` — сторож порядка укладки и разбора `texts` (одна и та же сущность у разных проектов обязана дать две отметки, а не одну); `a_repeated_heartbeat_within_the_same_batch_is_a_duplicate` — дедупликация внутри одного батча, а не только между батчами.
...
1. **Соответствие полей.** Тесты задачи 8 оставляли все необязательные поля пустыми, кроме проекта. При пустом поле курсор разбора не двигается — значит перестановка соседних полей в разборе (проект получает номер ветки) не ловилась ничем, и то же верно для десяти числовых и текстовых колонок, которые всюду были `None`. Тест `every_attribute_survives_the_round_trip` ниже обязателен: **все** поля заполнены **различимыми** значениями, и каждое поле, которое видно через `Attrs`, проверяется после чтения.

   Закрывается этим не весь долг, а его наблюдаемая часть. `wakode_core::Attrs` несёт девять полей плюс время; `plugin_id`, `is_write` и одиннадцать числовых и текстовых колонок в волне 0 не читает никто, и перестановка двух соседних `INTEGER` среди них не проявится ни в одном тесте. Заполнить их в тесте всё равно нужно — так проверяются сами позиции параметров вокруг них, — но остаток вынести в парковку явно, а не считать закрытым.
2. **Атомарность отката.** Тест на несуществующего пользователя в задаче 8 доказывал не то, что заявлял: внешний ключ срабатывает на первой же вставке, до `mark_dirty`, и пустой список дней получился бы и вовсе без транзакции. Тест `a_refused_batch_stores_no_heartbeats_at_all` ниже пиннит наблюдаемый итог — после отказа отметок не осталось, — но настоящей атомарности не доказывает и на этой схеме доказать не может: FK отбивает первую же вставку, так что записывать и откатывать оказывается нечего. Единственная мутация, которая его роняет, — выключить `PRAGMA foreign_keys`, а она роняет заодно и предсуществующий `heartbeat_for_a_missing_user_is_refused`. Долг остаётся открытым, а не закрытым; инвариант держится формой кода — один `tx`, один `commit`.
...
fn range_is_half_open() {
    ...
    // Но доказать этим тестом отсутствие сортировки нельзя: выборка идёт по
    // индексу `hb_time`, а обход по нему и так отдаёт строки по времени.
    // Тест роняет неверную явную сортировку (`ORDER BY time DESC`), но не её
    // исчезновение — отличить их можно только заглянув в план выполнения.
    let batch = [
...
fn every_attribute_survives_the_round_trip() {
    ...
    //
    // Закрывает тест время и девять полей `Attrs` — только их и можно
    // прочитать. `plugin_id`, `is_write` и одиннадцать числовых и текстовых
    // колонок заполняются, но обратно не читаются: их позиции в `INSERT` не
    // проверяет ничто, пока у них не появится читатель.
    let mut conn = open_in_memory().unwrap();
    migrate(&mut conn).unwrap();
    let user = insert_user(&conn, &a_user("swrneko")).unwrap();
    let interner = Interner::load(&conn).unwrap();
...
**Зачем.** SQLite допускает одного писателя. Если каждый HTTP-обработчик пишет сам, они дерутся за блокировку и получают `SQLITE_BUSY`. Единственная пишущая задача снимает эту борьбу для потока отметок — того, который в этой системе массовый.

**«Единственная» — про отметки, а не про базу.** Редкие одиночные записи (пользователи, ключи, сессии) в задаче 12 идут своими соединениями мимо этой очереди, и разводит их сам SQLite по `busy_timeout`. Это осознанно: логин или выдача ключа, застрявшие за чужим батчем отметок, — цена без выгоды. Долг, вынесенный из задачи 12 в финальное ревью: ни один тест не проверяет, что такая параллельная запись действительно дожидается `busy_timeout`, а не падает с `SQLITE_BUSY`.

**Групповой фиксации здесь нет.** Каждая заявка получает свой `insert_heartbeats` и свою транзакцию. Единственный писатель делает группировку *возможной* — все заявки уже проходят через одну точку, — но она не написана и в волну 0 не входит. Не ищи в коде батчинг, которого нет.
...
async fn writer_commits_and_reports() {
    ...
    // Отдельное соединение проверяет другое свойство — что закоммиченные
    // данные видны снаружи. «Отдельное» тут не авторский выбор: `conn` уже
    // переехал в `spawn_writer`, и читать им же не скомпилируется (E0382).
...
**Честно про определённость этого теста.** Пишущая задача живёт в настоящем потоке ОС и разгребает очередь параллельно с тем, как тест её наполняет, — значит железной гарантии отказа тут нет ни у какой формулировки, кроме той, что вводит шов для остановки писателя, а он несоразмерен. Тест опирается на запас: чтобы отказов не случилось ни одного, писатель обязан полностью закоммитить шестьдесят три батча в файловую базу быстрее, чем тестовая задача успеет выполнить шестьдесят четыре подряд идущих `try_send`. Это разница в несколько порядков, а не пограничный случай.

Из этого следуют два требования. Первое: комментарий в тесте обязан сказать именно это, а не изображать детерминизм. Второе: запас нужно **измерить**, а не предположить — прогнать тест десяток раз подряд (`cargo test -p wakode-store --test repository a_full_queue -- --exact` в цикле) и записать в отчёт, сколько отказов получалось. Если разброс упирается в ноль хоть раз — тест негоден в таком виде, и это находка, а не повод добавить `sleep`.
...
**Четвёртый тест, которого нет в блоке кода выше.** `backup_to_a_non_utf8_path_fails_instead_of_writing_a_differently_named_file` под `#[cfg(unix)]`: путь собирается через `OsStr::from_bytes(b"back\xffup.db")` — байт `0xFF` не встречается ни в одной валидной UTF-8 последовательности, а на Linux имена файлов legal как произвольные байты. `backup` обязан вернуть ошибку, и файла по запрошенному пути быть не должно. Без него `to_string_lossy` возвращается в код при первом же рефакторинге, а тихая потеря бэкапа — худший режим отказа, какой у резервного копирования бывает.

Run: `cargo test -p wakode-store --test repository`
Expected: PASS, четыре теста задачи 12.
