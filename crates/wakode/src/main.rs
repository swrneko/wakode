...
async fn run() -> anyhow::Result<()> {
    ...
    // унёс бы управление мимо `shutdown`. Для подкоманд это страховка —
    // `create_user` и `create_key` идут своими соединениями мимо очереди
    // писателя и коммитятся до возврата, — а для `serve` существенно:
    // через очередь идёт поток отметок.
