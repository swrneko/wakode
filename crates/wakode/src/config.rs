...
pub struct DurationsConfig {
    ...
    ///
    /// Ноль — и это **измеренное значение, а не заглушка**. Калибровка по
    /// живому аккаунту WakaTime показала, что добавки не существует
    /// вовсе: последняя отметка сессии не даёт ничего. Модель
    /// воспроизвела контрольный день до последнего знака — 62 интервала
    /// из 62, сумма сошлась и с `durations`, и с `grand_total`, и по
    /// каждому проекту. Метод и оговорки — в
    /// `.claude/docs/decisions/no-tail-padding.md`.
    ///
    /// Ключ оставлен: чужие цифры воспроизводятся нулём, но владелец
    /// вправе захотеть свои.
