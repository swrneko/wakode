...
/// один формат даты.
///
/// Внимание: это **нижняя** оценка дня, а не всегда тот день, под которым
/// момент подошьёт `split_by_local_day`. Там, где часы переводят назад после
/// полуночи и отматывают её (ньюфаундлендские зоны до 2011 года), заново
/// прожитый конец вчерашней даты лежит уже внутри новых суток, и нарезка
/// подошьёт его к следующему дню, а эта функция вернёт вчерашний. Если нужен
/// именно ключ дня — берите его из `split_by_local_day`, иначе цифры разойдутся
/// с посуточными итогами.
