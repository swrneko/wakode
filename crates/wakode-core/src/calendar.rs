...
///
/// Дата клампится в календарное окно — так же, как её клампит `local_date_of`,
/// иначе даты за окном возвращали бы полуинтервалы, пересекающиеся с краевым
/// днём, и «каждая дата владеет своими сутками» перестало бы быть правдой.
/// Крайние дни окна вбирают в себя всё время за его пределами: сутки
/// `0001-01-01` начинаются в `i64::MIN`, сутки `9999-12-31` кончаются в
/// `i64::MAX`.
pub fn local_day_bounds(date: NaiveDate, tz: Tz) -> (Micros, Micros) {
    let date = date.clamp(first_day(), last_day());
...
/// Дата, которую показывают стенные часы пользователя в этот момент.
///
/// Это **не** ответ на вопрос «за какой день засчитать это время»: у него есть
/// свой ответ — [`local_day_of`]. Стенные часы могут показывать вчерашнюю дату
/// уже внутри новых суток: там, где часы переводят назад после полуночи и
/// отматывают её (ньюфаундлендские зоны до 2011 года), заново прожитый конец
/// вчерашней даты физически лежит уже в следующем дне. Эта функция вернёт для
/// него вчерашнюю дату, а `split_by_local_day` подошьёт его к сегодняшней.
/// Отсюда правило: для показа времени — эта функция, для любых итогов —
/// [`local_day_of`]. Как оценка дня результат годится только снизу.
///
...
/// не обгонит момент.
///
/// Для любого `t < i64::MAX` возвращаемый конец строго позже `t`, а цикл
/// конечен: сутки за краем календарного окна насыщены до `i64::MAX`, поэтому
/// продвижение упирается в границу не позже последнего дня окна. Ровно на
/// `i64::MAX` проверка `day_end > t` на этом краю ложна, и продвижение вместо
/// остановки дошло бы до `NaiveDate::MAX` — десятки миллионов шагов. Этот вход
/// недостижим, но недостижим он благодаря вызывающей стороне: нарезка зовёт
/// функцию только под условием `cursor < iv.end`, откуда `cursor < i64::MAX`.
/// Сама функция от него не защищена, и новому вызывающему коду придётся
/// соблюдать то же условие.
...
/// Локальные сутки, за которые засчитывается момент времени.
///
/// Это **тот самый** день: та же дата, под которой момент окажется в
/// [`split_by_local_day`], и день, чьи [`local_day_bounds`] момент содержат.
/// Отвечать на этот вопрос через [`local_date_of`] нельзя — стенные часы
/// показывают вчерашнюю дату там, где перевод часов отматывает полночь, и
/// сводка за «сегодня» получила бы окно, в котором самого «сейчас» нет.
///
/// Время вне календарного окна насыщается к его краю — так же, как в
/// [`local_date_of`], и по той же причине.
pub fn local_day_of(t: Micros, tz: Tz) -> NaiveDate {
    // Кламп не косметический: `containing_day` продвигается по датам вперёд,
    // пока конец суток не обгонит момент, а на `i64::MAX` его не обгоняет
    // ничто. Внутри окна продвижение упирается в насыщенный конец последнего
...
///
/// Возвращается полуинтервал `[from, to)` — как и у [`local_day_bounds`], левая
/// граница включена, правая нет. Соглашение названо здесь явно, чтобы запрос в
/// хранилище писался по документации, а не выводился заново: в SQL это
/// `time >= from AND time < to`.
///
/// Полуинтервала хватает ровно: время суток приносит пара отметок, ранняя из
/// которых лежит в `[from, day_end)`, а поздняя — не дальше чем на таймаут от
/// неё, то есть строго раньше `to`. Отметка ровно в `to` способна составить
/// пару только с отметкой в `day_end`, а та принадлежит уже следующим суткам,
/// и весь их интервал — тоже. Поэтому включающая верхняя граница была бы столь
/// же корректна, просто бесполезна.
///
...
///
/// Стоимость линейна по числу пересечённых суток, и предохранителя тут нет:
/// интервал во весь диапазон `i64` даёт 3 652 059 кусков — в release это 1,4 с
/// и 1,3 ГБ памяти. Интервалы из `build_intervals` не длиннее настроенного
/// таймаута, но верхнюю границу самого таймаута `DurationConfig` не задаёт,
/// так что ограничение на длину интервала — забота вызывающей стороны.
...
mod tests {
    ...
    fn timestamps_outside_the_calendar_window_saturate_instead_of_panicking() {
        ...
        // Даты за окном не заводят собственных суток: иначе их полуинтервалы
        // пересекались бы с краевыми днями, и «каждая дата владеет своими
        // сутками» перестало бы выполняться.
        assert_eq!(local_day_bounds(date(20_000, 1, 1), tz), local_day_bounds(last_day(), tz));
        assert_eq!(local_day_bounds(date(-1000, 1, 1), tz), local_day_bounds(first_day(), tz));
    }
    ...
    #[test]
    fn authoritative_day_agrees_with_the_split_key_where_the_wall_clock_lies() {
        // 2009-11-01 в 03:01 UTC ньюфаундлендские зоны отматывали часы с 00:01
        // первого ноября на 23:01 тридцать первого октября. В 03:30 UTC стенные
        // часы показывают вчерашнюю дату, хотя сутки уже сменились.
        //
        // Обработчик `statusbar/today`, взявший день через `local_date_of` и
        // границы через `local_day_bounds`, получил бы окно, в котором самого
        // момента нет. `local_day_of` отвечает тем же днём, что и ключ нарезки.
        let tz: Tz = "America/Goose_Bay".parse().unwrap();
        let moment = at("2009-11-01T03:30:00Z");

        assert_eq!(local_day_of(moment, tz), date(2009, 11, 1));
        assert_eq!(local_date_of(moment, tz), date(2009, 10, 31), "стенные часы врут");

        // Ключ нарезки — то же самое число, и границы этого дня момент содержат.
        let iv = interval("2009-11-01T03:30:00Z", "2009-11-01T04:30:00Z");
        let split_key = *split_by_local_day(&[iv], tz).keys().next().unwrap();
        assert_eq!(local_day_of(moment, tz), split_key);

        let (day_start, day_end) = local_day_bounds(local_day_of(moment, tz), tz);
        assert!(day_start <= moment && moment < day_end);
    }

    #[test]
    fn authoritative_day_survives_the_edges_of_i64() {
        // `containing_day` продвигается по датам вперёд, и на `i64::MAX` без
        // насыщения дошёл бы до `NaiveDate::MAX` — десятки миллионов шагов.
        let tz: Tz = "America/Goose_Bay".parse().unwrap();
