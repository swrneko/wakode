...
/// Нижняя граница длины пароля администратора, в символах.
///
/// Восемь — не гигиена ради гигиены: этим паролем открывается учётная
/// запись с правами администратора на инстансе, который смотрит наружу, а
/// эндпоинт после создания закрывается навсегда — исправить «ой, я нажал
/// enter» через тот же экран будет уже нельзя. Верхней границы нет: длину
/// тела и так ограничивает axum, а argon2 длинный ввод переваривает.
