...
/// Заведомо неверное значение — не пустое, чтобы не совпасть по смыслу с
/// «не предъявлено» (см. докстринг `presented_token`), и не декодируемое
/// как base64 URL-safe без набивки (кириллица в алфавит не входит), чтобы
/// `matches` отказало ему при любом выданном токене.
...
///
/// Пустое (после обрезки пробелов) значение — наоборот, **не**
/// предъявление, а его отсутствие. Это не противоречит абзацу про мусор
/// выше: мусорное значение — это «я вставил токен и ошибся», пустое —
/// «поле стоит на форме, но заполнять его было незачем». Различие важно
/// для экрана настройки (план 4): `GET /api/setup/status` говорит ему
/// `token_required: false` на петлевой машине владельца, и форма,
/// отправляющая заголовок безусловно (пустой строкой, когда токен не
/// введён), не должна получать отказ по токену вместо обычного прохода
/// по адресу.
fn presented_token(headers: &HeaderMap) -> Option<String> {
    ...
    let presented = first.to_str().unwrap_or_default().trim().to_owned();
