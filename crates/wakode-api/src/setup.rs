...
/// - **Значение есть, но содержит байты вне видимого ASCII** (`to_str()`
///   вернул `Err`) — это предъявление, просто заведомо мусорное. Условие
///   шире, чем «не UTF-8»: `HeaderValue::to_str` отвергает любой
///   невидимый байт, поэтому корректная в UTF-8 кириллица попадает сюда
///   же. На поведение это не влияет — всё, что вне видимого ASCII, для
///   base64 мусор одинаково, — но называть условие уже, чем оно есть,
///   значит обещать читателю не то. Опасность здесь не
