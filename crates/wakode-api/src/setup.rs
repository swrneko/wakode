...
pub async fn setup(
    ...
    if !state.setup_from_any_address {
        ...
        // установка по-прежнему оставляет экран открытым. Доказательство
        // не в рассуждении: зеркальная половина
        // `a_loopback_peer_is_not_enough_when_the_request_came_through_a_proxy`
        // показывает, что запрос без единого заголовка проходит и отдаёт
        // `201` — а именно так и выглядит запрос через посредника,
        // который ничего не добавил.
        //
