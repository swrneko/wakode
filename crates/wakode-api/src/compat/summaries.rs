...
/// `seconds_including_other_language` на трёх фикстурах, — и дробные части
/// у всех шести меньше половины: `.2910, .2284, .0263, .4580, .0463,
/// .2284`. Любое правило даёт те же числа.
///
...
/// склеены в интервалы. Второй запрос дал бы то же число, что и
/// `heartbeats.first()`, ценой второго открытия соединения и второго
/// прохода по индексу.
///
...
pub async fn all_time_since_today(
    ...
    let first_day = heartbeats
        .first()
        .map_or(today, |hb| local_day_of(hb.time, tz));
