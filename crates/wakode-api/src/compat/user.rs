...
/// Видна соседям по `compat`: те же минуты стоят в `data.timeout` у
/// `all_time_since_today`, и вторая копия правила округления разъехалась бы
/// с этой молча — профиль показывал бы одну цифру, итог за всё время
/// другую.
pub(super) fn timeout_minutes(secs: i64) -> i64 {
    if secs <= 0 {
        return 0;
    }
