...
mod tests {
    ...
    fn the_base64_alphabet_is_pinned() {
        // Байты подобраны так, что их запись содержит `+` и `/` — символы,
        // которые в URL-safe алфавите заменены на `-` и `_`. Без этого
        // алфавит не пришпилен ничем: круговой обход согласован сам с
        // собой, а ключи из `openssl rand -base64 32` содержат хотя бы
        // один такой символ примерно в трёх случаях из четырёх. Смена
        // алфавита не подняла бы большинство существующих инстансов,
        ...
        assert_eq!(MasterKey::from_base64(ENCODED).unwrap().as_bytes(), &RAW);
        assert_eq!(
            MasterKey::from_base64(ENCODED).unwrap().to_base64(),
            ENCODED
        );
    }
