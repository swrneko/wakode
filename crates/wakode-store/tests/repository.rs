...
async fn a_full_queue_refuses_instead_of_buffering() {
    ...
    // задачи. Устойчивость держится не на запасе по времени, а на структуре:
    // `#[tokio::test]` даёт однопоточный рантайм, поэтому все 64 задачи
    // опрашиваются подряд без точки уступки перед `try_send` — планировщику
    // просто негде передать управление писателю посреди этого цикла. Чтобы
    // отказов не случилось ни одного, писателю пришлось бы закоммитить 63
    // файловых транзакции внутри этого же непрерывного окна, до того как
    // однопоточный рантайм вообще отдаст ему квант времени. Отсюда и
    // следствие: на медленной или однопроцессорной машине в CI число отказов
    // растёт, а не падает к нулю, — узкое место именно в однопоточности
    // тестового рантайма, а не в скорости писателя.
    assert!(
        refused > 0,
